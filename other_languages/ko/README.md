...
## 공지
Fusion 테마가 검색 엔진 최적화가 안되어있는 것을 발견했습니다. 테마는 사용자에게 잘 작동하고 있습니다. 그러나 구글 검색 엔진 콘솔을 확인한 결과, 자바스크립트를 이용한 모바일 디자인 변경이 검색 엔진에서는 적용이 안된다는 점을 확인했고, 검색엔진 최적화에 영향을 준다는 것을 발견했습니다. 밑에 리스트되어있는 블로그 테마는 사용하지 않을 것을 적극 권장합니다. Fusion 같은 경우에는 모바일 사용이 가능하게끔 추후 업데이트 할 예정입니다.
* Fusion
* RT
* Today
